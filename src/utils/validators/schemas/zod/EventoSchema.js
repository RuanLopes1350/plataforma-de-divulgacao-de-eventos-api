...
const midiaUploadValidationSchema = z.object({
  ...
  originalname: z.string()
      .optional()
      .refine((val) => !val || /\.(jpg|jpeg|png|gif|webp|mp4|avi|mov|wmv|flv|mkv|webm)$/i.test(val), {
          message: 'O arquivo deve ter extensão válida para imagem (.jpg, .jpeg, .png, .gif, .webp) ou vídeo (.mp4, .avi, .mov, .wmv, .flv, .mkv, .webm)'
      }),
  ...
  mimetype: z.union([
      ...
      message: 'O arquivo enviado não é uma mídia válida. Por favor, envie um arquivo de imagem (JPEG, PNG, GIF, WebP) ou vídeo (MP4, AVI, MOV, WMV, FLV, MKV, WebM).'
  }),
